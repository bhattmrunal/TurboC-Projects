...
void main()
{
  ...
  do /* restart game loop */
  {
    ...
    do
    {
      ...
      for (i=0;i<(speed*pause_length);i++)
       {
	int i=1+i; /*pause*/
       }
    ...
    gotoxy(30,9); textcolor(15); cprintf("Another Game (y/n)? ");
...
void add_segment()
{
  ...
  switch(direction)
  {
    ...
    case 'd': snake[snake_length].row=snake[snake_length-1].row;

		     snake[snake_length].col=snake[snake_length-1].col+1;

		     break;

    case 'a' : snake[snake_length].row=snake[snake_length-1].row;

		     snake[snake_length].col=snake[snake_length-1].col-1;

		     break;

    case 'w' : snake[snake_length].row=snake[snake_length-1].row-1;

		     snake[snake_length].col=snake[snake_length-1].col;

		     break;

    case 's' : snake[snake_length].row=snake[snake_length-1].row+1;
