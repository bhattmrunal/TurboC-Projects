...
void main(void)
{
 int gd=DETECT,gm,vertualx,vertualy,status,col,cur,k;
 int ch,x,y;
  //char far *egabase;
    ...
    switch(ch)
    {
     case 75:/* LEFT */
       ...
       case 60:pen=0; /* F2 - PEN UP */
	       break;
       case 61:save_icon();  // F3- Save icon
	      break;
       case 62:load();
	       //display_icon(0,IY);
	       //display_grid();
...
void display_help(void)
{
 // gotoxy(1,1);
 setcolor(9);
 outtextxy(10,10,"F1->PEN_DOWN");
 outtextxy(150,10,"F2->PEN_UP");
 outtextxy(280,10,"F3->SAVE");
 outtextxy(390,10,"F4->LOAD");
 outtextxy(500,10,"ESC->EXIT");
}
void load_grid()
{
 FILE *fp;
 int color=15,ix,iy,x,y;

  for(iy=0,y=144;y<144+136;y=y+4,iy++)
   {
    for(ix=0,x=240;x<240+160;x=x+4,ix++)
    {
     color=icon.image[ix][iy];
...
void load_icon()
{
 int icx,icy,color,posx,posy,len;
   ...
   if(icon.text!=NULL)
   {
    len=strlen(icon.text);
    settextstyle(0,0,0);
    setcolor(0);
    outtextxy(posx-(len*2)+7,posy+37,icon.text);
   }
...
void load()
{
 ...
 FILE *fp;
 gotoxy(5,20);
 printf("File name(to load : ");clreol();
 scanf("%s",fname);
 strcat(fname,".chn");
 gotoxy(5,20);clreol();
 setfillstyle(1,0);
 floodfill(300,310,0);
  if((fp=fopen(fname,"rb"))==NULL)
   ...
   fread(&icon,sizeof(struct icon_type),1,fp);
 load_icon();
 load_grid();
}
void ma_scr(void)
{
 frame(3,3,639-1,20,GREEN,LIGHTGREEN,1);
